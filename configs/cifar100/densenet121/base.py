...
def cifar100_pruning_and_task_parameters(p_exp_id, q_exp_id, kwargs):
    ...
    task_parameters = base.switch_quantize_prune(
        q_exp_id,
        p_exp_id,
        {
            'EPOCH': 15,
    ...
    if kwargs.get("no_layerwise", False):
        graduate_pruning["gradual_pruning"]["no_layerwise"] = True
    
    if kwargs.get("layer_pq", False):
        graduate_pruning["gradual_pruning"]["layer_pq"] = True

    return graduate_pruning, task_parameters


def setup(pa=0, pw=0, qa=0, qw=0, apoz='', sparse_aware=False, naive_p=False, quantize_mode='g', zg=False, zgf=False, structure=False, 
          bernoulli=False, mask_batchnorm=False, filter_p=None, unstructure_2nd=False, sp_balance=False, collect_q_stats="", 
          no_save=False, group_both=False, no_layerwise=False, q_then_p=False, layer_pq=False):
    ...
    if (q_exp_id != '' and p_exp_id != '' and (not q_then_p) and (not layer_pq)) or 'ust2' in p_exp_id:
        ...
        resume_from = None  # resume from pruning snaptshot
        # assert pw == pa
        if structure:
            resume_from = {
                50: './checkpoints/cifar100/densenet121/a50_mag_top_loc_st',
                75: './checkpoints/cifar100/densenet121/a75_mag_top_loc_st',
            }[pa]
    else:
    ...
    obj = {
        "name": f"cifar100/densenet121/{exp_id}",
        "task_parameters": {
            "gpu": True,
            "net": "densenet121",
            "warm": 0,
        ...
        "qsparse_parameters": {
            **gradual_pruning,
            ...
            "apoz": "/workspace/code/experiments/MDPI/analysis/apoz/json/cifar100_densenet.json" if apoz else ''
        }
    }
    return obj
