...
def convert(  # noqa: C901
    ...
    def is_builtin_name(name):
        return "quantize" in name.lower() or "prune" in name.lower()

    def mstr(m: nn.Module) -> str:
        if isinstance(m, nn.Sequential):
            if len(m) > 0:
                assert len(m) == 2
                return [a for a in [mstr(m[0]), mstr(m[1])] if not is_builtin_name(a)][0]
            else:
                return None
    ...
    def apply_operator(layer: Optional[nn.Module] = None) -> nn.Module:
        if layer is not None:
            ...
            r = copy.deepcopy(operator)
            # r.index = closure['count']
            # closure['count'] += 1
            return r
    ...
    def count_occurrence(
        ...
        def _count_occurrence(m: nn.Module, layer_type, scope):
            ...
            for name, layer in m.named_children():
                ...
                if exclude is not None and exclude in cur_scope:
                    continue
            ...
            if (not is_container(m)) or hasattr(m, "_qsparse_conversion"):
                ...
                if layer_type in activation_counter:
                    if (
                        ...
                        op = apply_operator()
                        if isinstance(op, PruneLayer):
                            if cur_scope in apoz:
                                op.sparsity = apoz[cur_scope]
                                apoz.pop(cur_scope)
                                logging.danger(f"assigning apoz sparsity {op.sparsity} to {cur_scope}")
                        # if spa: 
                        #     last_prune_layer = None
                        if order == "post":
                            m = nn.Sequential(m, op)
                        else:
                            m = nn.Sequential(op, m)
                        # if spa:
                        #     print("connecting quantization and pruning layer")
                        #     op.callback.set_p(last_prune_layer)
