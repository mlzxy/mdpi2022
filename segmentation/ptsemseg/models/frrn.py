...
class frrn(nn.Module):
    ...
    def __init__(self, n_classes=21, model_type="B", group_norm=False, n_groups=16, size=1.):
        ...
        if self.group_norm:
            self.conv1 = conv2DGroupNormRelu(3, S(48), 5, 1, 2)
        else:
            self.conv1 = conv2DBatchNormRelu(3, S(48), 5, 1, 2)

        up_residual_units = []
        down_residual_units = []
        for i in range(3):
            up_residual_units.append(
                RU(
                    channels=S(48),
                    kernel_size=3,
                    strides=1,
        ...
        self.up_residual_units = nn.Sequential(*up_residual_units)
        self.down_residual_units = nn.Sequential(*down_residual_units)

        self.split_conv = nn.Conv2d(S(48), 32, kernel_size=1, padding=0, stride=1, bias=False)
        ...
        self.merge_conv = nn.Conv2d(
            prev_channels + 32, S(48), kernel_size=1, padding=0, stride=1, bias=False
        )

        self.classif_conv = nn.Conv2d(
            S(48), self.n_classes, kernel_size=1, padding=0, stride=1, bias=True
        )
