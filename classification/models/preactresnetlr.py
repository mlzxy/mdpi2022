...
class PreActResNet(nn.Module):
    ...
    def __init__(self, block, num_block, class_num=100, size=1.0):
        super().__init__()
        self.input_channels = int(size * 64)

        self.pre = nn.Sequential(
            nn.Conv2d(3, int(size * 64), 3, padding=1),
            nn.BatchNorm2d(int(size * 64)),
            nn.LeakyReLU(inplace=True)
        )

        self.stage1 = self._make_layers(block, num_block[0], int(size * 64),  1)
        self.stage2 = self._make_layers(block, num_block[1], int(size * 128), 2)
        self.stage3 = self._make_layers(block, num_block[2], int(size * 256), 2)
        self.stage4 = self._make_layers(block, num_block[3], int(size * 512), 2)
...
def preactresnet18half():
    return PreActResNet(PreActBasic, [2, 2, 2, 2], size=0.5)

def preactresnet18quarter():
    return PreActResNet(PreActBasic, [2, 2, 2, 2], size=0.25)
